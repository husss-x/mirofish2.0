...
class GraphBuilderService:
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Fetch the complete graph data (including detailed information)

        Args:
            graph_id: Graph ID

        Returns:
            A dict containing nodes and edges, including timestamps, attributes, and other detailed data
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # Create a node map to look up node names
        node_map = {}
        for node in nodes:
