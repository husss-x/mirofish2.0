...
Uses an LLM to automatically generate detailed simulation parameters based on simulation requirements, document content, and knowledge graph information.
Achieves end-to-end automation with no manual parameter setup required.
...
class TimeSimulationConfig:
    ...
    # Time represented per round (simulated minutes) - default 60 minutes (1 hour) to speed up time flow
    minutes_per_round: int = 60

    # Range of Agent count activated per hour
    agents_per_hour_min: int = 5
    agents_per_hour_max: int = 20

    # Peak period (evening 19:00-22:00, most active time for Chinese users)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5

    # Off-peak period (midnight 0:00-5:00, almost no activity)
...
class SimulationConfigGenerator:
    ...
    automatically generating the optimal simulation parameter config
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # reserve 500 chars of headroom
...
- The user population is Chinese; schedules must conform to Beijing time habits
...
- Peak period in the evening 19:00–22:00 (activity multiplier: 1.5)
- Activity decreases after 23:00 (activity multiplier: 0.5)
- General pattern: low activity in the early morning, gradual increase in the morning, moderate during working hours, peak in the evening
- **Important**: The example values below are for reference only. You must adjust specific time windows based on the nature of the event and the characteristics of the participant groups.
  - For example: the student group peak may be 21:00–23:00; media is active all day; official institutions are only active during working hours
  - For example: a breaking hot topic may generate discussion late at night, so off_peak_hours can be shortened accordingly
...
- agents_per_hour_min (int): Minimum number of agents activated per hour (valid range: 1–{max_agents_allowed})
- agents_per_hour_max (int): Maximum number of agents activated per hour (valid range: 1–{max_agents_allowed})
- peak_hours (int array): Peak time periods; adjust based on event participant groups
- off_peak_hours (int array): Off-peak periods, typically late night / early morning
- morning_hours (int array): Morning time periods
- work_hours (int array): Working-hours time periods
- reasoning (string): Brief explanation of why this configuration was chosen"""

        system_prompt = "You are a social media simulation expert. Return pure JSON format. The time configuration must conform to Chinese daily schedule habits."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time config result and validate that agents_per_hour values do not exceed total agent count"""
        # Get raw values
...
For example: official statements should be posted by the Official/University type, news by MediaOutlet, student opinions by Student.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        """
        Assign the most suitable poster Agent for each initial post
        ...
        # Type alias map (handles different formats that LLM may output)
        type_aliases = {
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                ...
                if agent_configs:
                    # Sort by influence, select the highest-influence agent
...
- **Schedules must conform to Chinese daily habits**: almost no activity from 0:00–5:00 AM, most active from 19:00–22:00 in the evening
- **Official institutions** (University/GovernmentAgency): low activity (0.1–0.3), active during working hours (9–17), slow response (60–240 min), high influence (2.5–3.0)
...
{{
    "agent_configs": [
        {{
            "agent_id": <must match the input value>,
            "activity_level": <0.0–1.0>,
            "posts_per_hour": <posting frequency>,
            "comments_per_hour": <comment frequency>,
            "active_hours": [<list of active hours, accounting for Chinese daily schedule>],
        ...
        for i, entity in enumerate(entities):
            ...
            # If LLM didn't generate one, use rule-based generation
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            # Official institution: active during working hours, low frequency, high influence
            return {
                "activity_level": 0.2,
                "posts_per_hour": 0.1,
            ...
            # Expert/professor: active during work hours + evening, moderate frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
            ...
            # Student: mainly evenings, high frequency
            return {
                "activity_level": 0.8,
                "posts_per_hour": 0.6,
