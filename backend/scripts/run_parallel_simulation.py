...
- Environment remains open after simulation completes, entering command wait mode
...
# This fixes the problem where OASIS third-party libraries read files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Note: this ideally needs to be set at Python startup; setting it at runtime may not take effect
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        Wrap the open() function to use UTF-8 encoding by default for text mode.
...
def disable_oasis_logging():
    ...
    OASIS logging is too verbose (records every agent's observations and actions); we use our own action_logger instead.
    """
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize the logging configuration for the simulation.
...
# Available Twitter actions (INTERVIEW excluded; INTERVIEW can only be triggered manually via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (INTERVIEW excluded; INTERVIEW can only be triggered manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None/unspecified: interview both platforms simultaneously, returning a merged result
...
# Action type mapping table (database name -> canonical name)
ACTION_TYPE_MAP = {
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: The maximum rowid read on the last call (rowid is used instead of created_at
                    because created_at format differs between platforms)
        agent_names: agent_id -> agent_name mapping

    Returns:
        (actions_list, new_last_rowid)
        - actions_list: List of actions; each element contains agent_id, agent_name,
                        action_type, action_args (with context information)
    ...
    try:
        ...
        # Use rowid to track already-processed records (rowid is SQLite's built-in auto-increment field)
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    """
    # Check whether a boost configuration is available
    boost_api_key = os.environ.get("LLM_BOOST_API_KEY", "")
    boost_base_url = os.environ.get("LLM_BOOST_BASE_URL", "")
    boost_model = os.environ.get("LLM_BOOST_MODEL_NAME", "")
    has_boost_config = bool(boost_api_key)
    
    # Choose which LLM to use based on parameters and available config
    ...
    # If no model name is in .env, fall back to config
    if not llm_model:
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum number of simulation rounds (optional, used to cap long simulations)
    ...
    # Get the real agent name mapping from the config file (uses entity_name instead of the default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # For agents not in config, fall back to OASIS default names
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed row number in the database (using rowid avoids created_at format differences)
    ...
    for round_num in range(total_rounds):
        ...
        # Always log round start regardless of whether there are active agents
        if action_logger:
        ...
        # Fetch the actually executed actions from the database and log them
    ...
    # Note: do not close the environment; keep it available for Interview commands
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum number of simulation rounds (optional, used to cap long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close the environment immediately after simulation completes, without entering command wait mode'
    )
    ...
    if args.twitter_only:
        ...
        # Run in parallel (each platform uses its own independent logger)
    ...
    # Determine whether to enter command wait mode
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure a clean exit when SIGTERM/SIGINT is received.

    Persistent simulation scenario: the process does not exit after the simulation finishes;
    it waits for interview commands. When a termination signal is received, we need to:
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set the event to notify the asyncio loop to exit (giving it a chance to clean up)
            if _shutdown_event:
                _shutdown_event.set()

        # Do not call sys.exit() directly; let the asyncio loop exit normally and clean up.
        # Only force-exit if we receive the signal a second time.
        else:
            print("Force exiting...")
            sys.exit(1)
