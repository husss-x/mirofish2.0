  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card title (uses the first 20 characters of simulation requirement as title) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Simulation" and Step5 "Interaction" must be started live and do not support replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
const containerStyle = computed(() => {
  ...
  // Calculate the actual required height: rows * card height + (rows-1) * gap + small bottom margin
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    const x = offset * 35
    // Adjust starting position - close to the title but with appropriate spacing
...
// Format simulation_id display (take first 6 characters)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // If animating, only record the state and process after animation ends
        if (isAnimating) return

        // If the target state equals the current state, no action needed
        ...
        // Shorter delay when expanding (50ms), longer delay when collapsing (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if the pending state still needs to be applied (may have been overridden by later scroll)
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay a bit before executing to avoid switching too fast
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Adjust rootMargin: shrink from the bottom of the viewport so more scrolling is needed to trigger expansion
...
onMounted(async () => {
  // Ensure DOM has finished rendering before loading data
  await nextTick()
  await loadHistory()

  // Wait for DOM to render then initialize the observer
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
.grid-pattern {
  ...
  /* Anchored from the top-left; height changes only extend downward without shifting the existing grid */
...
.cards-container {
  ...
  /* min-height is dynamically calculated by JS, adapts based on card count */
}
