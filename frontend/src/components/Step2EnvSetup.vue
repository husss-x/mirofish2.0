    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using context, automatically invokes tools to map entities and relations from the knowledge graph, initializes simulation individuals, and endows them with unique behaviors and memories based on reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow speed, recommendation algorithms, active time windows per individual, posting frequency, event triggers, and other parameters based on simulation requirements and reality seeds
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish auto-plans <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> real-world hours, each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real time</span>
              </div>
...
// Log deduplication: track key info from last output
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Entered config generation phase — start polling for config
    if (!configTimer) {
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is at least 40 (recommended value) to avoid abnormal slider range
...
// Compute the total number of associated topics across all profiles
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User specified custom rounds — pass max_rounds param
    params.maxRounds = customMaxRounds.value
    addLog(`Starting simulation, custom rounds: ${customMaxRounds.value}`)
  } else {
    // User chose to keep auto-generated rounds — do not pass max_rounds param
...
onMounted(() => {
  // Auto-start the preparation flow
  if (props.simulationId) {
