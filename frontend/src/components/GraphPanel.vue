    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing. Refresh graph later.</span>
...
const renderGraph = () => {
  ...
  // Process edge data: count and index edges between each pair of nodes
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count edges between each node pair and collect self-loops
  tempEdges.forEach(e => {
  ...
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // Nodes whose self-loops have already been processed
  ...
  tempEdges.forEach(e => {
    ...
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly to ensure clear visual separation
      // Curvature range grows with edge count — more edges means wider spread
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // If the edge direction is opposite to the normalized direction, flip curvature
      // This ensures all edges are distributed in the same reference frame and don't overlap due to direction differences
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on number of edges between this node pair
    ...
    // Add gravitational pull toward center so isolated node clusters converge
  ...
  // Compute the curved path for a link
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const y2 = sy + 4
      // Draw the self-loop using an arc (sweep-flag=1 clockwise)
    ...
    // Perpendicular offset relative to the line, scaled to distance to keep curve clearly visible
  ...
  // Link labels background (white background makes text easier to read)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Record position only; do not restart simulation yet (distinguish click from drag)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // First real drag detected — now restart simulation
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only cool down the simulation if a real drag occurred
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions (no rotation — horizontal labels are clearer)
...
/* Style for the post-simulation finished hint */
