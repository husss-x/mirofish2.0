...
const resetAllState = () => {
  ...
  stopPolling()  // Stop any polling that may be running
}
...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Setting max rounds: ${props.maxRounds}`)
    }
...
// Track the last round per platform to detect changes and emit logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether the simulation has completed (via runner_status or platform completion flags)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: backend may not have updated runner_status yet, but platforms already report completion
      // Determine by checking twitter_completed and reddit_completed flags
...
const checkPlatformsCompleted = (data) => {
  // Return false if there is no platform data at all
  ...
  // If at least one platform is done, check whether all enabled platforms are done
  // Determine whether a platform is enabled by actions_count (count > 0 or was ever running)
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('Report request sent, please wait...')
    return
  }
